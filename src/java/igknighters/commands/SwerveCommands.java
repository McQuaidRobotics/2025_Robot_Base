  ...
  public static Command moveToSimple(CommandSwerveDrivetrain swerve, Pose2d targetPose) {
    ...
    return swerve.run(
      () -> {
        ...
        swerve.setControl(m_driveRequest.withVelocityX(xController.calculate(currentPose.getX(), targetPose.getX())).withVelocityY(yController.calculate(currentPose.getY(), targetPose.getY()))
            .withRotationalRate(thetaController.calculate(currentPose.getRotation().getRadians(), targetPose.getRotation().getRadians())));
      }
    );
  }
}
