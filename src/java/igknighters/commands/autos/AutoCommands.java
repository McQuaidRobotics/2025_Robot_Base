...
public class AutoCommands {
  ...
  protected Boolean withinTolerance(Pose2d pose, Pose2d target, double tolerance) {
    return pose.getTranslation().getDistance(target.getTranslation()) < tolerance
        && Math.abs(pose.getRotation().getDegrees() - target.getRotation().getDegrees()) < tolerance;
  }
  ...
  protected class GenericAuto{
    ...
    private Command finishAlignment(AutoTrajectory trajectory, double distOffset){
      if (trajectory.getFinalPose().isPresent()){
        Supplier<Command> cmdSup = 
            () -> {
              final Pose2d finalPose = trajectory.getFinalPose().get().plus(new Transform2d(distOffset, 0, Rotation2d.kZero));
              return loggedCmd(
                SwerveCommands.moveToSimple(swerve, finalPose)
                  .until(() -> withinTolerance(
                      SwerveCommands.getPose(swerve), finalPose, 0.1) && movingSlowerThan(swerve, .08).getAsBoolean())
              );
    ...
    public GenericAuto addDrivingTrajectory(Waypoints start, Waypoints end){
      final Timer moveTimer = new Timer();
      final AutoTrajectory trajectory = getTrajectory(start, end);
      if (!trajectorybeenadded) {
        
        headCommand.addCommands(trajectory.resetOdometry());
    }
