...
public class Stem implements ExclusiveSubsystem {
    ...
    public Stem() {
        if (Robot.isSimulation()) {
            ...
            pivot = new PivotReal();
            if (false) { // it will assume telescope is not broken
        ...
        if (!telescope.hasHomed()) {
            if (!position.isStow()) {
                DriverStation.reportWarning(
                        "Stem Telescope has not been homed, run stow to home", false);
                return false;
            }
    ...
    /**
     * Meant as the main api for controlling the stem, this method takes in a {@link StemPosition}
     * and sets the stem to that position. This method will return false if any of the mechanisms
     * have not yet reached their target position.
     *
     * @param position The position to set the stem to
     * @return True if all mechanisms have reached their target position
     */
     ...
     * @param toleranceMult A value to multiply the accepted positional tolerance by @@return True
     *     if all mechanisms have reached their target position
     */
    ...
    /**
     * Control each component with voltage control. This is NOT recommended for general use and
     * should only be used to test the mechanisms.
     *
