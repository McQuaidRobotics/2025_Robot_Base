...
public class StemValidator {
    ...
    public static class MechanismPoints {
        ...
        public MechanismPoints(StemPosition stemPosition) {
            ...
            Translation2d wristAxelPointTransform =
                    new Translation2d(
                            stemPosition.getTelescopeMeters()
                                    * Math.cos(stemPosition.getPivotRads()),
                            stemPosition.getTelescopeMeters()
                                    * Math.sin(stemPosition.getPivotRads()));
            this.wristAxelPoint =
                    kRobotCollisionGeometry.PIVOT_LOCATION.plus(wristAxelPointTransform);
    ...
    /**
     * Returns true if the passed in x value is inebtween the two planes that make the verticle
     * lines of the side of the rectangle.
     */
    ...
    /**
     * Returns an {@link InvalidationResponse} that defines if the position is valid and if it's
     * not, how its invalid.
     */
    public static ValidationResponse validatePosition(StemPosition stemPosition) {
        ...
        boolean isUmbrellaRightPointsBelowDriveBase =
                (mechPts.umbrellaBottomRightPoint.getY()
                                <= kRobotCollisionGeometry.DRIVE_BASE.getTopLeft().getY()
        ...
        boolean stemCollidesDrivebase =
                isWristAxelPointBelowDrivebase
    ...
    public static StemPosition stepTowardsTargetPosition(
        ...
        if (!pivotMovementValidReason.isValid()
            ...
            if (!pivotMovementValidReason.equals(ValidationResponse.COLLIDES_BOUNDS)) {
                ...
                double difference =
                        Math.abs(
                                lowestOfUmbrellaPoints
                                        - kRobotCollisionGeometry.DRIVE_BASE.getTopY());
                double opposite =
                        (currentState.telescopeMeters * Math.sin(invalidPosition.getPivotRads()))
                                + difference;
                ...
                midStatePivotRads =
                        Math.asin(pivotAxelToWristAxelSlope.getY() / midStateTelescopeMeters);
            }
        }

        StemPosition midState =
                StemPosition.fromRadians(
                        midStatePivotRads, midStateWristRads, midStateTelescopeMeters);
