...
public class Umbrella implements ExclusiveSubsystem {
  ...
  //     Consumer<Pose2d[]> noteSender = poses -> s.send(new Localizer.NamedPositions("Notes",
  // poses));
  ...
  //         this::isIntaking,
  //         notes -> {
  //             Pose2d[] notePoses = notes.stream().map(p -> new Pose2d(p,
  // GeomUtil.ROTATION2D_ZERO)).toArray(Pose2d[]::new);
