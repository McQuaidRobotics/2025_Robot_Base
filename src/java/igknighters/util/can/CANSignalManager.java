...
public class CANSignalManager {
    ...
    private static final HashMap<String, HashMap<SignalType, ArrayList<BaseStatusSignal>>>
            signalsDatabase = new HashMap<>(64);
        ...
        for (BaseStatusSignal signal : signals) {
            ...
            if (name.contains(VOLT_NAME)) {
                ...
                if (!map.containsKey(SignalType.CONTROL))
                    map.put(SignalType.CONTROL, new ArrayList<>());
    ...
    public static void refreshSignals() {
        for (Entry<String, HashMap<SignalType, ArrayList<BaseStatusSignal>>> entry :
                signalsDatabase.entrySet()) {
            ...
            if (map.containsKey(SignalType.CONTROL)) list.addAll(map.get(SignalType.CONTROL));
            if (CHARACTERIZATION_MODE.get()) {
                list.addAll(map.get(SignalType.VOLT));
                list.addAll(map.get(SignalType.AMP));
            } else {
                if (map.containsKey(TYPE_THIS_CYCLE)) list.addAll(map.get(TYPE_THIS_CYCLE));
            }
            if (list.size() != 0)
    ...
    public static void setCharacterizationMode(boolean mode) {
        ...
        for (Entry<String, HashMap<SignalType, ArrayList<BaseStatusSignal>>> entry :
            ...
            HashMap<SignalType, ArrayList<BaseStatusSignal>> map = entry.getValue();
            if (map.containsKey(SignalType.VOLT)) list.addAll(map.get(SignalType.VOLT));
            if (map.containsKey(SignalType.AMP)) list.addAll(map.get(SignalType.AMP));

            BaseStatusSignal.setUpdateFrequencyForAll(
                    freq, list.toArray(new BaseStatusSignal[list.size()]));
        }
    }
}
